...
namespace MusicCreater
{
    public partial class Form1 : Form
    {
        ...
        {
            if (File.Exists((string)music_name)) {
                this.outputDevice = new WaveOutEvent();
                this.audioFile = new AudioFileReader((string)music_name);
                this.outputDevice.Init(this.audioFile);
                this.outputDevice.Volume = ((float)this.audio_volume.Value / 10);
        ...
        private void record_btn_Click(object sender, EventArgs e)
        {
            if (this.record_btn.Text == "Записать") // Start Record:
            {
                ...
                string outputFilePath = (string)AppDomain.CurrentDomain.BaseDirectory + "/RecordAudio/system_recorded_audio.wav";

                if (!Directory.Exists((string)AppDomain.CurrentDomain.BaseDirectory + "/RecordAudio"))
                {
                    Directory.CreateDirectory((string)AppDomain.CurrentDomain.BaseDirectory + "/RecordAudio");
                }
